...
HAVE_OSC=""; HAVE_EXO=""
command -v oapi-cli >/dev/null 2>&1 && HAVE_OSC=1 || skip "oapi-cli is not installed; Outscale sits out this comparison"
command -v exo      >/dev/null 2>&1 && HAVE_EXO=1 || skip "exo is not installed; Exoscale sits out this comparison"
...
if [ "$participants" -lt 2 ]; then
  ...
  [ -z "$verdict" ] || fail "the same request produced different machines: $rows (interfaces/addresses)"
...
[ -z "${o// /}" ] || fail "scaleway carries unpublished address(es):$o"
if [ -n "$HAVE_OSC" ]; then
  o="$(orphans_of "$osc_m" "$osc_priv")"
  [ -z "${o// /}" ] || fail "outscale carries unpublished address(es):$o"
fi
if [ -n "$HAVE_EXO" ]; then
  o="$(orphans_of "$exo_m" "$exo_priv")"
  [ -z "${o// /}" ] || fail "exoscale carries unpublished address(es):$o"
fi
ok "zero orphan addresses"
...
if [ "$participants" -lt 2 ]; then
  ...
  [ -z "$verdict" ] || fail "with one public address each, the machines differ: $rows (interfaces/addresses)"
  ok "one request, one shape: $rows (interfaces/addresses)"
fi
