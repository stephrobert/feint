...
399 operations served across 3 packs, counted from the record of the last
...
|---|---|---|---|---|---|---|---|---|
| Exoscale | 104 | 88 % (91) | 92 % (96) | 92 % (96) | 88 % (91) | 32 % (33) | 80 % (83) | 17 % (18) |
| Outscale | 100 | 100 % (100) | 100 % (100) | 100 % (100) | 100 % (100) | 94 % (94) | 89 % (89) | 96 % (96) |
| Scaleway | 195 | 96 % (187) | 98 % (192) | 100 % (195) | 96 % (187) | 51 % (100) | 89 % (173) | 72 % (141) |
| **All three** | 399 | 95 % (378) | 97 % (388) | 98 % (391) | 95 % (378) | 57 % (227) | 86 % (345) | 64 % (255) |
...
be closed on it. So the 17 % and the 96 % at the two ends of that column are
...
401 routes across 3 packs. Every route names the upstream operation it
...
|---|---|---|---|
| `DELETE` | `/account/v3/projects/{id}` | `account/v3/ProjectAPI.DeleteProject` | `client` `contract` `runtime` `probe-refusal` `behaviour` `negative` |
...
| `PATCH` | `/account/v3/projects/{id}` | `account/v3/ProjectAPI.UpdateProject` | `client` `contract` `runtime` `probe` `behaviour` |
| `POST` | `/account/v3/projects` | `account/v3/ProjectAPI.CreateProject` | `client` `contract` `runtime` `probe` `behaviour` |

### `baremetal`

| Method | Path | Upstream operation | Proven by |
|---|---|---|---|
| `GET` | `/baremetal/v1/zones/{zone}/offers` | `baremetal/v1/API.ListOffers` | `client` `contract` `runtime` `probe` |
| `GET` | `/baremetal/v1/zones/{zone}/servers` | `baremetal/v1/API.ListServers` | `client` `contract` `runtime` `probe` |
...
| `GET` | `/instance/v1/zones/{zone}/products/volumes` | `instance/v1/API.ListVolumesTypes` | `no-client` `contract` `probe` |
...
| `POST` | `/instance/v1/zones/{zone}/servers` | `instance/v1/API.CreateServer` | `client` `contract` `shape` `runtime` `probe` `behaviour` `negative` |
...
| `POST` | `/instance/v2alpha1/zones/{zone}/servers/{id}/detach-private-network-interface` | `instance/v2alpha1/API.DetachServerPrivateNetworkInterface` | `client` `contract` `runtime` `probe` `behaviour` |
...
| `DELETE` | `/lb/v1/zones/{zone}/backends/{backendID}/servers` | `lb/v1/ZonedAPI.RemoveBackendServers` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| `GET` | `/lb/v1/zones/{zone}/lb-types` | `lb/v1/ZonedAPI.ListLBTypes` | `client` `contract` `runtime` `probe` |
| `GET` | `/lb/v1/zones/{zone}/lbs/{lbID}/backend-stats` | `lb/v1/ZonedAPI.ListBackendStats` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| `GET` | `/lb/v1/zones/{zone}/lbs/{lbID}/certificates` | `lb/v1/ZonedAPI.ListCertificates` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| `GET` | `/lb/v1/zones/{zone}/lbs/{lbID}/stats` | `lb/v1/ZonedAPI.GetLBStats` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| `GET` | `/lb/v1/zones/{zone}/subscribers` | `lb/v1/ZonedAPI.ListSubscriber` | `client` `contract` `runtime` `probe` |
| `PATCH` | `/lb/v1/zones/{zone}/ips/{ipID}` | `lb/v1/ZonedAPI.UpdateIP` | `client` `contract` `shape` `runtime` `probe` `behaviour` `negative` |
| `POST` | `/lb/v1/zones/{zone}/backends/{backendID}/servers` | `lb/v1/ZonedAPI.AddBackendServers` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| `POST` | `/lb/v1/zones/{zone}/lbs/{lbID}/migrate` | `lb/v1/ZonedAPI.MigrateLB` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| Method | Path | Upstream operation | Proven by |
|---|---|---|---|
| `GET` | `/metadata` | `scw/Client.GetAPIMetadata` | `client` `contract` `runtime` `probe` |
...
| `DELETE` | `/vpc/v2/regions/{region}/routes/{routeID}` | `vpc/v2/API.DeleteRoute` | `client` `contract` `runtime` `probe-refusal` `behaviour` `negative` |
...
| `GET` | `/vpc/v2/regions/{region}/routes/{routeID}` | `vpc/v2/API.GetRoute` | `client` `contract` `shape` `runtime` `probe-refusal` `behaviour` `negative` |
...
| `PATCH` | `/vpc/v2/regions/{region}/routes/{routeID}` | `vpc/v2/API.UpdateRoute` | `client` `contract` `shape` `runtime` `probe-refusal` `behaviour` `negative` |
...
| `POST` | `/vpc/v2/regions/{region}/routes` | `vpc/v2/API.CreateRoute` | `client` `contract` `shape` `runtime` `probe-refusal` `behaviour` `negative` |
...
### Served, and driven by no client (8)
...
- `instance` — 1 operation — the official CLI answers this from a table compiled into the binary and sends no request, measured against a dead port on 2026-09-14; the client that reaches it is the Ansible collection of #758, which this suite does not run
...
- `account` — 1 operation — it deletes a project together with everything filed under it, and the resources of this pack are removed through their own per-product paths: a sweep written here would be a second inventory of what exists, and the kind it forgets is the one that outlives a delete the client was told succeeded
- `baremetal` — 8 operations — it attaches a bare-metal server to a Private Network, and a seeded server holds no NIC this pack's VPC could carry: the one measured inventory declares it joins no private network for this product (joins_private_networks = False), and an attach served without the network behind it would publish a membership nothing routes
- `baremetal` — 7 operations — it reads one offer, the OS and options an offer accepts, or a partitioning schema, and nothing here is in stock: the catalogue ListOffers answers is what the seeded fleet declares and nothing more (baremetal.go), an OS or an option describes an install this emulator does not perform, and a partitioning schema does too
- `baremetal` — 5 operations — it acts on the hardware behind a server — an install writes an OS to its disks, a reboot, a start and a stop cycle its power, a delete returns it to stock — and there is no hardware behind a seeded server for any of that to happen to
- `baremetal` — 4 operations — it edits a seeded server — its name, tags and protection, the reverse of one of its addresses, the options it subscribes to — and the seed is the operator's file: an edit accepted here would be undone by the next restore, or would silently diverge from what the operator declared
- `baremetal` — 3 operations — BMC access is a remote console on the hardware's management controller, opened with a password the cloud generates for the session; there is no controller behind a seeded server and no session to open on it
- `baremetal` — 3 operations — it reads one server by identifier, its events or its metrics, and the measured clients never do: an inventory enumerates (the stephrobert.scaleway plugin's own comment says it never reads one by one), and the events and metrics of a seeded server would describe a life it did not live here
- `baremetal` — 3 operations — ordering Elastic Metal is a paid commitment on hardware the cloud delivers in minutes to hours, and a server here is seeded through the state door, never ordered: an order this emulator accepted would be a delivery nobody makes
- `baremetal` — 2 operations — the product's settings are per-project switches on the real account, one type in the SDK today (smtp: whether a project's servers may send mail), and nothing here sends anything
...
- `lb` — 4 operations — nothing here terminates TLS: a certificate served by this emulator would be an ID over key material that signs nothing, and the Let's Encrypt half issues against domains this emulator does not hold; the list is served since #666, and it is empty, because that is what a balancer here holds
...
- `vpcgw` — 1 operation — no client this project drives upgrades a gateway in place, which is what this whole block records; the capacity argument that used to sit here was withdrawn with MigrateLB's in #762
...
| Method | Path | Upstream operation | Proven by |
|---|---|---|---|
| `POST` | `/api/v1/CreateFlexibleGpu` | `osc/Client.CreateFlexibleGpu` | `client` `contract` `runtime` `probe` `behaviour` `negative` |
| `POST` | `/api/v1/DeleteFlexibleGpu` | `osc/Client.DeleteFlexibleGpu` | `client` `contract` `runtime` `probe` `behaviour` `negative` |
| `POST` | `/api/v1/LinkFlexibleGpu` | `osc/Client.LinkFlexibleGpu` | `client` `contract` `runtime` `probe` `behaviour` `negative` |
| `POST` | `/api/v1/ReadFlexibleGpuCatalog` | `osc/Client.ReadFlexibleGpuCatalog` | `client` `contract` `shape` `runtime` `probe` |
| `POST` | `/api/v1/ReadFlexibleGpus` | `osc/Client.ReadFlexibleGpus` | `client` `contract` `runtime` `probe` `behaviour` |
| `POST` | `/api/v1/UnlinkFlexibleGpu` | `osc/Client.UnlinkFlexibleGpu` | `client` `contract` `runtime` `probe` `behaviour` `negative` |
| `POST` | `/api/v1/UpdateFlexibleGpu` | `osc/Client.UpdateFlexibleGpu` | `client` `contract` `runtime` `probe` `behaviour` `negative` |
...
| `POST` | `/api/v1/ReadLoadBalancers` | `osc/Client.ReadLoadBalancers` | `client` `contract` `shape` `runtime` `probe` `behaviour` `negative` |
...
- `DedicatedGroup` — 4 operations — dedicated hosts are hardware this station does not have, and the templates and groups above them are proprietary orchestration, out of scope here for the same reason as baremetal
...
- `VmGroup` — 6 operations — dedicated hosts are hardware this station does not have, and the templates and groups above them are proprietary orchestration, out of scope here for the same reason as baremetal
- `VmTemplate` — 4 operations — dedicated hosts are hardware this station does not have, and the templates and groups above them are proprietary orchestration, out of scope here for the same reason as baremetal
...
| `GET` | `/v2/block-storage-snapshot/{id}` | `exoscale/v2.get-block-storage-snapshot` | `client` `contract` `runtime` `probe-refusal` `behaviour` |
...
| `GET` | `/v2/elastic-ip/{id}` | `exoscale/v2.get-elastic-ip` | `client` `contract` `runtime` `probe` `behaviour` |
| `GET` | `/v2/elastic-ip` | `exoscale/v2.list-elastic-ips` | `client` `contract` `shape` `runtime` `probe` `behaviour` |
| `GET` | `/v2/instance-pool/{id}` | `exoscale/v2.get-instance-pool` | `client` `contract` `runtime` `probe` `behaviour` |
...
| `GET` | `/v2/load-balancer/{id}` | `exoscale/v2.get-load-balancer` | `client` `contract` `runtime` `probe` `behaviour` |
...
| `GET` | `/v2/security-group/{id}` | `exoscale/v2.get-security-group` | `client` `contract` `shape` `runtime` `probe` `behaviour` |
...
| Method | Path | Upstream operation | Proven by |
|---|---|---|---|
| `GET` | `/v2/operation/{id}` | `exoscale/v2.get-operation` | `client` `contract` `shape` `runtime` `probe` |
