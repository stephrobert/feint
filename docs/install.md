...
Version **0.11.0**, named rather than fetched as `latest`: a mutable reference
...
```bash
base=https://github.com/stephrobert/feint/releases/download/v0.11.0
...
```bash
gh release download v0.11.0 --repo stephrobert/feint --pattern 'feint-linux-amd64'
...
```bash
docker run --rm -p 127.0.0.1:4599:4599 ghcr.io/stephrobert/feint:v0.11.0
...
```yaml
services:
  - name: ghcr.io/stephrobert/feint:v0.11.0
    alias: feint
```
...
```bash
cosign verify ghcr.io/stephrobert/feint:v0.11.0 \
  --certificate-identity-regexp '^https://github\.com/stephrobert/feint/\.github/workflows/release\.yml@refs/tags/v' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

gh attestation verify oci://ghcr.io/stephrobert/feint:v0.11.0 \
