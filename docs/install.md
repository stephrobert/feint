...
```bash
docker run --rm -p 127.0.0.1:4599:4599 ghcr.io/stephrobert/feint:v0.12.1
...
```yaml
services:
  - name: ghcr.io/stephrobert/feint:v0.12.1
    alias: feint
```
...
```bash
cosign verify ghcr.io/stephrobert/feint:v0.12.1 \
  --certificate-identity-regexp '^https://github\.com/stephrobert/feint/\.github/workflows/release\.yml@refs/tags/v' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

gh attestation verify oci://ghcr.io/stephrobert/feint:v0.12.1 \
