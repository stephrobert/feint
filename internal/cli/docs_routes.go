...
func evidenceLegend(evidence *evidenceArtefact) string {
	...
	b.WriteString("**An injected fault earns none of these.** The emulator can be made to refuse\n")
	b.WriteString("on purpose (`PUT /_feint/faults`), and an answer produced that way moves no\n")
	b.WriteString("counter here: the operation stays un-driven, un-checked and un-proven, and a\n")
	b.WriteString("`negative` span cannot be closed on it. An operation whose only refusal came\n")
	b.WriteString("from a rule somebody armed has not been proven, it has been staged — so the\n")
	b.WriteString("injected answers are counted apart, under `injected` on\n")
	b.WriteString("`/_feint/conformance`, and `tools/conformance/score.sh` refuses a run that\n")
	b.WriteString("carries any.\n\n")
