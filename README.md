...
>
> **Unknown**: 18 operations are mounted and have never been driven by a client. Every one of them states why no official client reaches it, at the route and in [docs/routes.md](docs/routes.md). They are counted rather than glossed, one by one, in [coverage/evidence.json](coverage/evidence.json).
>
...
Version **0.8.0**, named rather than fetched as `latest`: a mutable reference
...
```bash
gh release download v0.8.0 --repo stephrobert/feint --pattern 'feint-linux-amd64'
...
272 routes are mounted across the three packs. The tables count *upstream operations*
...
85 routes mounted. Of the 374 operations upstream declares: 22% served,
67% declined on purpose, 9% untriaged.

| Group | Served | Declined | Untriaged | Upstream |
|---|--:|--:|--:|--:|
| `dbaas` | 0 | 146 | 0 | 146 |
| `compute` | 66 | 10 | 35 | 111 |
| `sks` | 0 | 25 | 0 | 25 |
| `ai` | 0 | 22 | 0 | 22 |
| `iam` | 0 | 18 | 0 | 18 |
| `kms` | 0 | 16 | 0 | 16 |
...
85 routes mounted. Of the 263 operations upstream declares: 32% served,
67% declined on purpose, 0% untriaged.
