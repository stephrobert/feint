...
>
> **Proven**: 364 of the 389 mounted operations are driven by a real client, on every pull request: each pack's own official CLI, and an infrastructure engine wherever a pack admits one, all of them running against the emulator in CI, and machines really boot: an ssh login on each provider's own default account, isolated subnets, a firewall that filters. The whole chain is described in [docs/conformance.md](docs/conformance.md).
>
> **Not proven**: quotas, prices, real capacity, identifier validation, authentication, eventual consistency. The 54 sections of [docs/limits.md](docs/limits.md) each say what one costs. An emulator with a single implicit account and no price list would have to invent those figures, and somebody would act on them.
>
...
The image is control-plane only and carries no `latest` tag; [docs/install.md](docs/install.md) has the GitLab form, the compose form and the signature verification. Pull it directly with `docker run --rm -p 127.0.0.1:4599:4599 ghcr.io/stephrobert/feint:v0.12.1`.
...
Version **0.12.1**, named rather than fetched as `latest`: a mutable reference
...
```bash
base=https://github.com/stephrobert/feint/releases/download/v0.12.1
...
```bash
gh release download v0.12.1 --repo stephrobert/feint --pattern 'feint-linux-amd64'
...
389 routes are mounted across the three packs. The tables count *upstream operations*
...
185 routes mounted. Of the 498 operations upstream declares: 37% served,
62% declined on purpose, 0% untriaged.
...
100 routes mounted. Of the 263 operations upstream declares: 38% served,
61% declined on purpose, 0% untriaged.
