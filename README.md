...
The image is control-plane only and carries no `latest` tag; [docs/install.md](docs/install.md) has the GitLab form, the compose form and the signature verification. Pull it directly with `docker run --rm -p 127.0.0.1:4599:4599 ghcr.io/stephrobert/feint:v0.12.0`.
...
Version **0.12.0**, named rather than fetched as `latest`: a mutable reference
...
```bash
base=https://github.com/stephrobert/feint/releases/download/v0.12.0
...
```bash
gh release download v0.12.0 --repo stephrobert/feint --pattern 'feint-linux-amd64'
