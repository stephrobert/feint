...
- **Two recordings, on the page rather than in the repository** (#252). The
  ...
  the binary on a laptop, the other a GitHub Actions job pulling the image —
  the two doors, each beside the snippet it belongs to, in both READMEs.
  Generated by `mise run demo` from tapes in the repository, so a command that
  breaks breaks the video, and `tools/demo/ci.sh` proves the sequence before
  `vhs` films it. Watch it here:
  [the CI path](https://github.com/stephrobert/feint/blob/main/docs/assets/ci.gif),
  [the laptop path](https://github.com/stephrobert/feint/blob/main/docs/assets/quickstart.gif).
